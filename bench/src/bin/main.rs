...
async fn process_all_csv_files(input_bucket: &str, output_bucket: &str) -> Result<(), Box<dyn std::error::Error>> {
...
async fn upload_histogram(bucket: &str, key: &str, hist: Hist) -> Result<(), Box<dyn std::error::Error>> {
    let region = Region::from_str("us-east-2")?; 
    let s3_client = S3Client::new(region);
