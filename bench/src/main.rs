...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dists: Vec<(String, Option<u32>)> = match t_mode.as_str(){
        "MM" => {
            match mode.as_str(){
                "Vec" => {
                    test_cases::nmm(split[0].parse::<usize>().unwrap(), split[1].parse::<usize>().unwrap(), split[2].parse::<usize>().unwrap(), split[3].parse::<usize>().unwrap(), "Vec".to_string())
                },
                "Stack" => {
                    test_cases::nmm(split[0].parse::<usize>().unwrap(), split[1].parse::<usize>().unwrap(), split[2].parse::<usize>().unwrap(), split[3].parse::<usize>().unwrap(), "Stack".to_string())
                }
                _ => Vec::new(),
            }
        },
